...
# Step 2: Plot time vs x for each ID (vehicle) with different colors
def plot_time_x_for_each_lane_and_id(df):
    df = df.sort_values(by=['time_step', 'id'])
    lanes = df['lane_number'].unique()
    norm = plt.Normalize(vmin=0, vmax=30)
    for lane in lanes:
        lane_data = df[df['lane_number'] == lane]

        plt.figure(figsize=(10, 6))

        # Loop through each vehicle (ID) in this lane
        for vehicle_id in lane_data['id'].unique():
            vehicle_data = lane_data[lane_data['id'] == vehicle_id]

            # Create scatter plot and save the object for the colorbar
            scatter = plt.scatter(vehicle_data['time_step'], vehicle_data['position_x'],
                                  c=vehicle_data['speed'], cmap='viridis', norm=norm, label=f'Vehicle {vehicle_id}',
                                  s=2)

        # Add color bar based on the scatter plot
        plt.colorbar(scatter, label='Speed (0 to 30)')  # Add color bar based on the scatter plot

        plt.title(f'Time vs Position (X) for Lane {lane}')
