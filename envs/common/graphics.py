...
class EnvViewer:
    ...
    def display(self) -> None:
        ...
        # Only render pedestrians if the 'pedestrians' attribute exists and is not None
        if hasattr(self.env, 'pedestrians') and self.env.pedestrians is not None:
            PedestrianGraphics.display(self.env.pedestrians, self.sim_surface, offscreen=self.offscreen)
...
class FixedCameraEnvViewer(EnvViewer):
    """Custom viewer with a fixed camera for the environment."""
    ...
    def set_bounds(self, left, right, bottom, top):
        """Set the fixed bounds for the camera (viewport)."""
        self.left = left
        self.right = right
        self.bottom = bottom
        self.top = top
    ...
    def window_position(self) -> np.ndarray:
        """Fix the camera at the stored position."""
        return np.array([self.x, self.y])

    def draw_circle(self, position, radius, color):
        """Draw a circle with a given radius and color at a specific position."""
        # Convert pedestrian position to pixel coordinates using sim_surface
        pix_x, pix_y = self.sim_surface.pos2pix(position[0], position[1])
        # Draw a circle representing the pedestrian at the correct pixel position
        pygame.draw.circle(self.sim_surface, color, (int(pix_x), int(pix_y)), radius)
