...
class AbstractEnv(gym.Env):
    ...
    def save_vehicle_info_csv(self, filename="vehicle_info.csv"):
        """Save all available information about vehicles on the road to a CSV file."""
        if not self.road or not self.road.vehicles:
            print("No road or vehicles available.")
            return

        with open(filename, 'w', newline='') as csvfile:
            fieldnames = ['vehicle', 'attribute', 'value']
            writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
            writer.writeheader()

            for v in self.road.vehicles:
                # Save the basic vehicle info (class and position)
                writer.writerow({'vehicle': str(v), 'attribute': 'class', 'value': str(type(v))})
                writer.writerow({'vehicle': str(v), 'attribute': 'position', 'value': str(v.position)})

                # Save all non-callable attributes of the vehicle
                for attr in dir(v):
                    if not callable(getattr(v, attr)) and not attr.startswith("__"):
                        writer.writerow({'vehicle': str(v), 'attribute': attr, 'value': getattr(v, attr)})

    def log_trajectories(self):
        """Log the ID, time, position, lane number, speed, and acceleration of all vehicles at the current step."""
        if not self.road or not self.road.vehicles:
            print("No road or vehicles available.")
            return

        for v in self.road.vehicles:
            # Retrieve the vehicle ID directly (assuming it exists in the object, such as v.id)
            vehicle_id = getattr(v, 'id', f"{id(v)}")  # Use the numeric part only
            vehicle_id_numeric = ''.join(filter(str.isdigit, vehicle_id))

            # Retrieve the position (x, y)
            position_x = v.position[0]
            position_y = v.position[1]

            # Retrieve the lane number from the lane index (last element if it's a list)
            lane_number = v.lane_index[-1] if isinstance(v.lane_index, (list, tuple)) else v.lane_index

            # Retrieve the speed
            speed = getattr(v, 'speed', None)

            # Compute or retrieve the acceleration if possible
            if hasattr(v, 'acceleration'):
                acceleration = v.acceleration
            else:
                acceleration = None  # Or calculate it based on the speed over time

            # Log the trajectory
            self.trajectories.append({
                'id': vehicle_id_numeric ,
                'time_step': self.time,
    ...
    def save_trajectories_csv(self, filename="test_trajectories.csv"):
        """Save the logged trajectories to a CSV file."""
        with open(filename, 'w', newline='') as csvfile:
            fieldnames = ['id', 'time_step', 'position_x', 'position_y', 'lane_number', 'speed', 'acceleration']
            writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
            writer.writeheader()
    ...
    def render(self):
        ...
        # Add this check before rendering pedestrians
        if hasattr(self, 'pedestrians') and self.pedestrians:
            # Ensure pedestrians are only rendered if enabled in the config
