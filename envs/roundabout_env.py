...
from MixTrafficSimulation.road.lane import CircularLane, LineType, SineLane, StraightLane
...
class RoundaboutEnv(AbstractEnv):
    ...
    def step(self, action: int) -> tuple[np.ndarray, float, bool, bool, dict]:
        ...
        self.sim_time += 1
        # Continuously generate vehicles at the edges
        self._generate_vehicles_continuously()

        return obs, reward, terminated, truncated, info

    def _generate_vehicles_continuously(self):
        """Generate vehicles continuously during the simulation at random intervals."""
        if len(self.road.vehicles) < self.config.get("max_vehicles", 200):
            self._make_vehicles(continuous=True)

    def _make_vehicles(self, continuous=False, interval=10) -> None:
        """
        Populate a road with several vehicles on the highway and on the merging lane, as well as an ego-vehicle.
        If continuous is True, only new vehicles will be added at the edges, without creating the ego-vehicle again.
        :param continuous: Boolean flag to control continuous vehicle generation.
        """
        position_deviation = 2
        speed_deviation = 2

        if not continuous:
            # Ego-vehicle (created only during initial population)
            ego_lane = self.road.network.get_lane(("eer", "ees", 0))
        ...
        # Define edge lanes for incoming vehicles
        edge_lanes = [
            ("wer", "wes", 0),  # West entry
            ("eer", "ees", 0),  # East entry
            ("ser", "ses", 0),  # South entry
            ("ner", "nes", 0),  # North entry
        ]

        # Define exit lanes as destinations
        exit_lanes = [
            "wxr",  # West exit
            "exr",  # East exit
            "sxr",  # South exit
        ...
        # Add vehicles at the edges (only during continuous generation)
        if continuous:
            other_vehicles_type = utils.class_from_path(self.config["other_vehicles_type"])
            if self.sim_time % interval == 0:
                for lane in edge_lanes:
                    if len(self.road.vehicles) < self.config.get("max_vehicles", 200):  # Control max number of vehicles
                        ...
                        # Select a random exit as the destination
                        destination = self.np_random.choice(exit_lanes)
    ...
    def render(self, mode: str = "human"):
        ...
        self.viewer.set_bounds(-300, 300, -300, 300)  # Adjust bounds for the merge area
