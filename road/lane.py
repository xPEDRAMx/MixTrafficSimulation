...
from MixTrafficSimulation.road.spline import LinearSpline2D
from MixTrafficSimulation.utils import Vector, class_from_path, get_class_path, wrap_to_pi
...
class CrosswalkLane:
    """A class representing a crosswalk made of multiple straight lanes (hatched appearance)."""

    def __init__(self, start: np.ndarray, end: np.ndarray, stripe_length: float = 2.0, gap: float = 1.0, width: float = AbstractLane.DEFAULT_WIDTH):
        """
        Initialize a CrosswalkLane consisting of multiple straight lanes with a gap between them.

        :param start: Starting position of the crosswalk [m]
        :param end: Ending position of the crosswalk [m]
        :param stripe_length: Length of each stripe in the crosswalk [m]
        :param gap: Gap between each stripe [m]
        :param width: Width of the crosswalk [m]
        """
        self.start = start
        self.end = end
        ...
        self.gap = gap
        self.width = width
        self.lanes = self._create_hatched_crosswalk()

    def _create_hatched_crosswalk(self):
        """Create multiple `StraightLane` segments to form a hatched crosswalk."""
        total_length = np.linalg.norm(self.end - self.start)
        direction = (self.end - self.start) / total_length

        # Get the direction perpendicular to the lane
        perpendicular_direction = np.array([-direction[1], direction[0]])  # Rotate direction 90 degrees

        lanes = []

        # Calculate the number of stripes that will fit within the total crosswalk width (across the lane)
        num_stripes = int(total_length // (self.stripe_length + self.gap))

        for i in range(num_stripes):
            # Calculate the center of each stripe
            stripe_center = self.start + i * (self.stripe_length + self.gap) * direction

            # Move outwards by half of the crosswalk width along the perpendicular direction
            stripe_start = stripe_center - (self.width / 2) * perpendicular_direction
            stripe_end = stripe_center + (self.width / 2) * perpendicular_direction

            # Create a straight lane for each stripe and add to the list
            lane = StraightLane(start=stripe_start, end=stripe_end, width=self.width,
                                line_types=[LineType.CONTINUOUS, LineType.CONTINUOUS])
            lanes.append(lane)

        return lanes

    def add_to_road(self, road_network: RoadNetwork, start_id: str, end_id: str):
        """Add the crosswalk's individual stripes to the road network."""
        for i, lane in enumerate(self.lanes):
            road_network.add_lane(f"{start_id}_{i}", f"{end_id}_{i}", lane)
